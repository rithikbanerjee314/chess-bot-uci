...
#   ENGINE_CMD=../build/bin/crucible   # path to the compiled UCI binary
...
ENGINE_CMD="${ENGINE_CMD:-$REPO_DIR/build/bin/crucible}"
TC="${TC:-30+0.3}"
ROUNDS="${ROUNDS:-15}"
...
ENGINE_ARGS=(-engine cmd="$ENGINE_CMD" name=Crucible)
for elo in $ELOS; do
  ENGINE_ARGS+=(-engine cmd="$STOCKFISH_CMD" option.UCI_LimitStrength=true "option.UCI_Elo=$elo" option.Threads=1 "name=SF-$elo")
done

echo "Gauntlet: Crucible vs Stockfish @ [$ELOS], tc=$TC, $ROUNDS rounds/opponent, concurrency=$CONCURRENCY"
