...
Crucible is a standalone [UCI](https://en.wikipedia.org/wiki/Universal_Chess_Interface)
...
Produces `build/bin/crucible` (the UCI engine) and `build/bin/perft` (the
...
2. Run `gauntlet/run-gauntlet.sh`. It pits the compiled engine against
   ...
   STOCKFISH_CMD=/path/to/stockfish CUTECHESS_CMD=/path/to/cutechess-cli \
     ENGINE_CMD=./build/bin/crucible ./gauntlet/run-gauntlet.sh
   ```
...
3. Get an anchored Elo estimate from the PGN. If you have
   ...
   ```
   node gauntlet/compute-elo.js gauntlet/results/<file>.pgn Crucible \
